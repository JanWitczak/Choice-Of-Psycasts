...
namespace ChoiceOfPsycasts
{
	...
	public static class AbilityLibrary
	{
		...
		public static int MaxLevel = Math.Min(10, (int) HediffDefOf.PsychicAmplifier.maxSeverity);
		static AbilityLibrary()
		{
			...
			{
				if (Ability.abilityClass == typeof(Psycast) && Ability.level > 0 && Ability.level <= MaxLevel)
				{
		...
		{
			if (i > 0 && i <= MaxLevel) return true;
			else return false;
		}
		public static bool ProperLevelRange(LevelRange i)
		{
			if (i.low > 0 && i.low <= (int)HediffDefOf.PsychicAmplifier.maxSeverity && i.high >= i.low && i.high <= MaxLevel) return true;
			else return false;
		}
	}
